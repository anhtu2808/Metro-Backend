...
@Slf4j
public abstract class AbstractService<E, C, U, R> implements IService<E, C, U, R> {

    protected JpaRepository<E, Long> repository;
    protected EntityMappers<E, C, U, R> entityMapper;

    protected AbstractService(JpaRepository<E, Long> repository, EntityMappers<E, C, U, R> entityMapper) {
    ...
    public PageResponse<R> findAll(int page, int size, String arrange) {
        log.info("Fetching entities of type with page: {}, size: {}, sort: {}", page, size, arrange);
    ...
    public R update(Long id, U request) {
        ...
        E oldEntity = repository.findById(id).orElseThrow(() -> {
            log.error("Entity with ID: {} not found", id);
            return new AppException(ErrorCode.ENTITY_NOT_FOUND);
        });
